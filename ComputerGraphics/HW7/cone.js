export class Cone {
    ...
    constructor(gl, segments = 32, options = {}) {
        ...
        for (let i = 0; i < segments; i++) {
            ...
            // face의 중앙 각도(midAngle) 기준으로 바깥쪽을 가리키는 (cos, 1 = tan(pi/4) * 1, sin)
            const nx = Math.cos(midAngle) / Math.sqrt(2);
            const ny = 1.0 / Math.sqrt(2);
            const nz = Math.sin(midAngle) / Math.sqrt(2);
